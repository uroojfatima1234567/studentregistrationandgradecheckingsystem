...
public class MarksLinear extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MarksLinear frame = new MarksLinear();
	...
	public MarksLinear() {
		...
		getcourse.setText("Linear Algebra");
		...
		courseth.setBounds(879, 31, 161, 22);
		panel.add(courseth);
		...
		btnNewButton_1.setBounds(953, 650, 90, 30);
		...
		try 
		{
			...
			Conn c=new Conn();	//Object class Conn Database
			String query="SELECT faculty_id, faculty_name, course_name from faculty join courses using(course_id) where course_id=127";
			ResultSet rs= c.s.executeQuery(query);
